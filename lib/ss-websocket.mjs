    ...
    async function process(data) {
      ...
      for (const chunk of decode(Buffer.from(data))) {
        if (!writer) {
          ...
          // connect(host, port, true) dials the relay; retried once when the
          // direct socket fails or closes silently (Cloudflare-hosted targets).
          const open = async (viaRelay) => {
            if (socket) { try { writer?.releaseLock(); } catch {} try { socket.close(); } catch {} }
            try { socket = await connect(host, port, viaRelay); }
            catch (error) { if (!viaRelay && connect.canRelay) return open(true); throw error; }
            if (viaRelay || !connect.canRelay) socket.closed?.catch(close);
            if (closed) { await socket.close(); return; }
            clearTimeout(timer); writer = socket.writable.getWriter();
            void (async () => {
              const reader = socket.readable.getReader();
              let received = false;
              try { while (!closed) { const { value, done } = await reader.read(); if (done) break; received = true; ws.send(encode(value)); } }
              catch (error) { if (received || viaRelay || !connect.canRelay) throw error; }
              finally { reader.releaseLock(); }
              if (!received && !viaRelay && connect.canRelay && !closed) return open(true);
              close();
            })().catch(close);
